...
- `template.html` / `template.js` — printable US Letter landscape sheet (v2) with four ARUCO_MIP_36h12 markers and a 6 in bar. Print at 100 % (Actual size).
...
1. **Your sheet.** Tape-measure the 6 in line on the printed sheet and confirm that length (checkbox, or type the actual length between 5.5 and 6.5 in). That value is remembered on the phone. Plain US Letter paper is the fallback.
...
Follow [TEST-PROTOCOL.md](TEST-PROTOCOL.md) on the phone’s own browser (Safari on iPhone, Chrome on Android), not an in-app browser. Tape-measure the door first, print the sheet at 100 %, tape the 6 in line, then take three readings and paste Copy results into your notes.
